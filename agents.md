...
Phase 8 adds difficult-document handling only: deterministic page/document quality detection, `qualitySummary`/page `qualitySignals`, review-required document-quality tasks, selective page-level visual embeddings, `worker-visual-embeddings` on the `visual-embeddings` queue, visual search mode plus explicit hybrid `includeVisual`, Qwen handwriting/degraded-document routing that defaults candidates to `needs_review`, and difficult-document cues in Search, Viewer, and Review Queue. Do not implement Phase 9 analysis/chat, synthesis, comparison, notes, answer generation, or analysis workers while finishing Phase 8 validation.
...
7. The Qwen3-VL-4B smart profile uses `max_model_len=24576`, `max_num_seqs=2`, video disabled, `gpu_memory_utilization=0.84`, and four page images per semantic request by default to match the old 2B semantic fan-in shape. Smart Parse images are planner-resolution only through Qwen's 32x guidance: 256 minimum and 2560 maximum visual tokens per image (`STRUCTURA_VLLM_MM_PROCESSOR_KWARGS={"size":{"shortest_edge":262144,"longest_edge":2621440}}`). Do not downscale Docling originals globally or weaken Granite page/crop/table inputs. Qwen semantic planner prompts should omit token-heavy Docling bboxes and page image hashes; Qwen needs page/element/table IDs plus text/table context for routing, while Granite receives the grounded page/crop/table extraction inputs. If a multi-image response fails exact Docling page coverage or the active vLLM service rejects the request for context length, retry that window as one-page requests, merge with whole-document Docling context, and record the fallback reason in manifest confidence.
...
3. Visual retrieval is contractually part of `/api/v1/search` through `mode=visual` and `includeVisual`; Phase 9 should call the search service instead of creating a divergent retrieval planner.
4. The `visual-embeddings` queue and `worker-visual-embeddings` Compose profile are separate from text embeddings, extraction, relationships, and future analysis workers; Phase 9 should add `worker-analysis` separately.
5. Phase 8 UI evidence lives under `docs/ui-reference/figma/difficult-documents/` with the deterministic Linux snapshot `tests/e2e/phase8.spec.ts-snapshots/phase8-difficult-documents-chromium-linux.png`.
...
docs/ui-reference/figma/difficult-documents/
```
...
Writable runtime object/cache/log/tmp trees must remain `10001:bgconley` with setgid directories and group-readable blobs so both containers and GPU-node operator tools can read derived artifacts. App and worker containers must include the host operator GID through `STRUCTURA_RUNTIME_HOST_GID`/Compose `group_add` so non-root UID `10001` can preserve setgid inheritance. Repair permission drift with `infrastructure/zfs/create_gpu_runtime_datasets.sh`; do not route diagnostics around the object store to compensate for bad modes.
...
Playwright milestone validation must target the GPU-hosted web service, not a Mac-hosted Vite server. The Mac can act as the browser/controller, but the app under test must be served from the GPU node on the LAN. Current live UI target is `http://10.25.0.50:13000` with `STRUCTURA_E2E_LIVE=1`; the GPU Compose `.env` should expose only web externally via `STRUCTURA_WEB_BIND_HOST=0.0.0.0` and `STRUCTURA_WEB_PORT=13000`, while keeping API and Postgres bound to `127.0.0.1`. The canonical live browser milestone suite must include every implemented phase live spec: `tests/e2e/phase1-live.spec.ts`, `tests/e2e/phase2-live.spec.ts`, `tests/e2e/phase3-live.spec.ts`, `tests/e2e/phase4-live.spec.ts`, `tests/e2e/phase5-live.spec.ts`, `tests/e2e/phase6-live.spec.ts`, `tests/e2e/phase7-live.spec.ts`, and `tests/e2e/phase8-live.spec.ts`.
