...
7. The Qwen3-VL-4B smart profile uses `max_model_len=16384`, `max_num_seqs=2`, video disabled, `gpu_memory_utilization=0.84`, and four page images per semantic request by default to match the old 2B semantic fan-in shape. If a multi-image response fails exact Docling page coverage, retry that window as one-page requests, merge with whole-document Docling context, and record the fallback reason in manifest confidence.
