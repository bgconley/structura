...
Writable runtime object/cache/log/tmp trees must remain `10001:bgconley` with setgid directories and group-readable blobs so both containers and GPU-node operator tools can read derived artifacts. Repair permission drift with `infrastructure/zfs/create_gpu_runtime_datasets.sh`; do not route diagnostics around the object store to compensate for bad modes.
