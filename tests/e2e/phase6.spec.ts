...
test("Phase 6 automation workbench manages contacts, rules, watcher settings, and suggestions", async ({page}) => {
  ...
  await page.getByLabel("Action tag").fill("insurance");
  ...
  await expect(page.getByText("document_family eq medical_eob")).toBeVisible();
  ...
  await expect(page.getByText("/srv/structura/imports/incoming")).toBeVisible();
