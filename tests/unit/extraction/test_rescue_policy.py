...
def test_rescue_policy_returns_review_only_when_removed_rescue_permission_is_present() -> None:
    decision = RescuePolicy().decide(
        RescuePolicyContext(
    ...
    assert decision.outcome == "review_only"
    assert "Separate semantic rescue has been removed" in decision.reason
