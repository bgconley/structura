...
- `lib/semantic_annotations/prompting.py` — Owns Qwen semantic-planner prompt assembly and prompt-version constants; Smart Parse v3 is bounded-recall, forbids canonical facts, includes compact hard-class examples, and keeps Docling context compact (~1200 tok)
- `tests/unit/semantic_annotations/test_prompting.py` — Prompt-contract regression tests proving Smart Parse asks for material-region recall, page inventory, Docling grounding, planner metadata, and excludes old sparse-target wording (~450 tok)
- `tests/fixtures/semantic_annotations/semantic_canary_expectations.example.json` — Committed example shape for private semantic-canary expectations: required/forbidden document families, semantic types, target schemas, continuation groups, full-page flags, and region attributes (~600 tok)
